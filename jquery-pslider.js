/* This Slider is modified from the Article https://tympanus.net/codrops/2012/03/15/parallax-content-slider-with-css3-and-jquery/
License: GPL V2
*/
(function(jQuery,undefined){jQuery.Slider=function(options,element){this.jQueryel=jQuery(element);this._init(options);};jQuery.Slider.defaults={current:0,bgincrement:50,autoplay:true,interval:4000};jQuery.Slider.prototype={_init:function(options){this.options=jQuery.extend(true,{},jQuery.Slider.defaults,options);this.jQueryslides=this.jQueryel.children('div.da-slide');this.slidesCount=this.jQueryslides.length;this.current=this.options.current;if(this.current<0||this.current>=this.slidesCount){this.current=0;}
...
this._updatePage();this._loadEvents();if(this.options.autoplay){this._startSlideshow();}},_navigate:function(page,dir){var jQuerycurrent=this.jQueryslides.eq(this.current),jQuerynext,_self=this;if(this.current===page||this.isAnimating)return false;this.isAnimating=true;var classTo,classFrom,d;if(!dir){(page>this.current)?d='next':d='prev';}
else{d=dir;}
...
if(!this.cssAnimations||!this.cssAnimations){jQuerynext.css('left',(d==='next')?'100%':'-100%').stop().animate({left:'0%'},1000,function(){_self.isAnimating=false;});jQuerycurrent.stop().animate({left:(d==='next')?'-100%':'100%'},1000,function(){jQuerycurrent.removeClass('da-slide-current');});}
...
this._navigate(idx);},_loadEvents:function(){var _self=this;this.jQuerypages.on('click.cslider',function(event){_self.page(jQuery(this).index());return false;});this.jQuerynavNext.on('click.cslider',function(event){if(_self.options.autoplay){clearTimeout(_self.slideshow);_self.options.autoplay=false;}
var page=(_self.current<_self.slidesCount-1)?page=_self.current+1:page=0;_self._navigate(page,'next');return false;});this.jQuerynavPrev.on('click.cslider',function(event){if(_self.options.autoplay){clearTimeout(_self.slideshow);_self.options.autoplay=false;}
var page=(_self.current>0)?page=_self.current-1:page=_self.slidesCount-1;_self._navigate(page,'prev');return false;});if(this.cssTransitions){if(!this.options.bgincrement){this.jQueryel.on('webkitAnimationEnd.cslider animationend.cslider OAnimationEnd.cslider',function(event){if(event.originalEvent.animationName==='toRightAnim4'||event.originalEvent.animationName==='toLeftAnim4'){_self.isAnimating=false;}});}
else{this.jQueryel.on('webkitTransitionEnd.cslider transitionend.cslider OTransitionEnd.cslider',function(event){if(event.target.id===_self.jQueryel.attr('id'))
...
else{this.each(function(){var instance=jQuery.data(this,'cslider');if(!instance){jQuery.data(this,'cslider',new jQuery.Slider(options,this));}});}
return this;};})(jQuery);

jQuery(window).on('load resize', function () {		
	jQuery('#da-slider').cslider({autoplay:true,bgincrement:450});
	var siteWidth = jQuery('#site-container').width();
	var resWidth = jQuery('#resmwdt').width();
	if ( resWidth > 8 ) {
		jQuery('.da-slider').height(siteWidth*0.357575);					
		jQuery('.da-slide .da-img, .da-slide .da-texlink').height(jQuery('.da-slider').height()-60);
	}				
	else {
		jQuery('.da-slider').height(siteWidth*0.67);
		jQuery('.da-slide .da-img').height(jQuery('.da-slider').height()*0.55);
		jQuery('.da-slide .da-texlink').height(jQuery('.da-slider').height()*0.45-70);
	}
